...
# Function calculates the sum of distances from the given city order
# array - distance matrix
# order - order of cities


def get_distance(array, order):
    dist = array[order[order.size - 1], order[0]]
    for i in range(len(order) - 1):
        dist += array[order[i + 1], order[i]]
    return dist
...
# Function checks for the normal_swap and array_swap variants whether a given swap is on the taboo list
...
# taboo - taboo list
# punish - penalty value for the swap
# k, m - coordinates for which to set the punishment value
...
# Function swaps positions in the list
# order - list in which to change the order
# i - coordinates of the city index to be swapped
# j - coordinates of the index where the city will be inserted
...
# array - distance matrix between cities
    ...
    for i in range(0, order.size - 1):
        for j in range(i + 1, order.size):
            if is_not_on_taboo(order[i], order[j], taboo):
                ...
                if get_distance(array, tmpOrder) < bestLoopDistance:
                    bestLoopDistance = get_distance(array, tmpOrder)
...
# Main function that provides individual best distances for the variant where a fixed number of iterations is given
# punishment - penalty for the swap
# reps - number of iterations
# bestOrder - initial order of cities
...
# Note: Data should
df = pd.read_excel("Dane_PFSP_50_20.xlsx")
...
# MainFoo(5,1000,bestOrder,get_distance(bestOrder))
